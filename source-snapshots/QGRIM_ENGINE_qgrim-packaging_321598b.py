...
#!/usr/bin/env python3
"""
QGRIM v2.1 — Unified Quantum Circuit Engine  [Reconstructed Engineering Edition]
===============================================================
Self-contained runtime module. Runs on Python 3.8+ including Pydroid (Android); tests and provenance are distributed separately.
...
Features:
  ...
  • 19 built-in circuits: Bell, GHZ, QFT, teleportation, Deutsch, Bernstein-Vazirani,
...
Usage (Pydroid):
  ...
  No pip install or internet is required for direct built-in execution; tests and provenance are separate files.
...
def _validate_qubits(qubits: List[int], lineno: int, mnem: str) -> None:
    """Reject operands outside the simulator's four-qubit domain."""
    for q in qubits:
        if not (0 <= q < QUBITS):
            raise AsmError(
                f"line {lineno}: {mnem} qubit {q} out of range "
                f"(valid 0-{QUBITS - 1})"
            )
...
def assemble(source: str) -> List[int]:
    ...
    Software-extension (sim only, not FPGA):
      ...
      RZ  q idx     — RZ rotation (same encoding as PHASE)
            IFX m q        — simulator-only X(q) if measurement m == 1
      IFZ m q        — simulator-only Z(q) if measurement m == 1
      MARK basis     — simulator-only phase flip of one basis state
      REFLECT basis  — simulator-only reflection: +1 on basis, -1 elsewhere
    ...
    for lineno, raw in enumerate(source.splitlines(), 1):
        ...
        if mnem == "S":
            ...
            _validate_qubits(args[:1], lineno, mnem)
            ...
            _validate_qubits(args[:2], lineno, mnem)
            ...
            _validate_qubits(args[:3], lineno, mnem)
            ...
            c, t = args[0], args[1]
            _validate_qubits([c, t], lineno, mnem)
        ...
        elif mnem in ("IFX", "IFZ"):
            # Simulator-only measurement-controlled corrections. These use
            # reserved nonzero NOP fields; 0x0000 remains a raw NOP.
            if len(args) < 2:
                raise AsmError(f"line {lineno}: {mnem} needs measurement and target qubits")
            _validate_qubits(args[:2], lineno, mnem)
            cond_code = 1 if mnem == "IFX" else 2
            out.append(_encode(0x0, args[0], args[1], cond_code))
        elif mnem in ("MARK", "REFLECT"):
            # Simulator-only four-qubit basis operators. These use reserved
            # NOP fields and are an engineering extension, not hardware ISA.
            if len(args) < 1:
                raise AsmError(f"line {lineno}: {mnem} needs a basis index")
            if not (0 <= args[0] < STATES):
                raise AsmError(
                    f"line {lineno}: {mnem} basis must be in range 0-{STATES - 1}"
                )
            op_code = 3 if mnem == "MARK" else 4
            out.append(_encode(0x0, args[0], 0, op_code))
...
def disassemble(words: List[int]) -> List[str]:
    ...
    for pc, w in enumerate(words):
        ...
        if op == 0x0 and imm in (1, 2, 3, 4):
            mnem = {1: "IFX", 2: "IFZ", 3: "MARK", 4: "REFLECT"}[imm]
        # format operands
        if op in (0x0, 0x6, 0xF) and not (op == 0x0 and imm in (1, 2, 3, 4)):  # NOP, INIT, HALT
            ops = ""
        elif op in (0x1, 0x2, 0x4, 0x8, 0xA):  # H X MEASURE LOAD_AMP Y
            ops = f"q{a}"
        elif op in (0x3, 0x7, 0xB):  # CNOT SWAP CZ
            ops = f"q{a}, q{b}"
        elif op == 0x0 and imm in (1, 2):  # IFX/IFZ [SW]
            ops = f"m{a}, q{b}"
        elif op == 0x0 and imm in (3, 4):  # MARK/REFLECT [SW]
            ops = f"|{a:04b}⟩"
        ...
        sw = "  [SW]" if (op in (0xA, 0xB, 0xC, 0xD) or (op == 0x0 and imm in (1, 2, 3, 4))) else ""
...
def _validate_noise_probability(noise_p: float) -> float:
    """Validate and normalize the public depolarizing probability input."""
    if isinstance(noise_p, bool) or not isinstance(noise_p, numbers.Real):
        raise TypeError("noise_p must be a real number")
    value = float(noise_p)
    if not math.isfinite(value) or not 0.0 <= value <= 1.0:
        raise ValueError("noise_p must be finite and within [0.0, 1.0]")
    return value


def _validate_nonnegative_integer(value: int, name: str, minimum: int = 0) -> int:
    """Validate an integer count at a public API boundary."""
...
def _validate_qubit_index(q: int) -> int:
    """Validate a public four-qubit index."""
    if isinstance(q, bool) or not isinstance(q, int):
        raise TypeError("qubit index must be an integer")
    if q < 0 or q >= QUBITS:
        raise ValueError(f"qubit index must be in range 0-{QUBITS - 1}")
    return q


def _validate_state_vector(state, name: str) -> List[complex]:
    """Validate a finite state vector without requiring exact unit norm."""
    try:
        values = list(state)
    except TypeError as exc:
        raise TypeError(f"{name} must be a sequence of numeric values") from exc
    if len(values) != STATES:
        raise ValueError(f"{name} must contain exactly {STATES} elements")
    converted = []
    for value in values:
...
class QGRIMSim:
    ...
    _noise_active: bool = field(default=False, init=False, repr=False)

    def __post_init__(self):
        self.noise_p = _validate_noise_probability(self.noise_p)
    ...
    def clone(self) -> "QGRIMSim":
        ...
        s._noise_active = self._noise_active
    ...
    def _apply_noise(self, qubits: List[int]):
        """Apply one non-recursive depolarizing decision per affected qubit."""
        if self.noise_p <= 0.0 or self._noise_active:
            return
    ...
    def step(self, instr: int) -> bool:
        ...
        if   op == 0x0:
            if imm == 1 and self.measurements.get(a, 0) == 1:
                self._pauli_x(b)
            elif imm == 2 and self.measurements.get(a, 0) == 1:
                self._phase(b, 8)                       # conditional Z
            elif imm == 3:
                self.state[a] = -self.state[a]          # MARK basis [SW]
            elif imm == 4:
                for i in range(STATES):
                    if i != a:
                        self.state[i] = -self.state[i]  # REFLECT basis [SW]
    ...
    def entanglement_entropy(self, partition_a: Optional[List[int]] = None) -> float:
        ...
        if partition_a is None:
            ...
            try:
                ...
                raise TypeError(
                    "partition_a must be an iterable of integer qubit indices"
                ) from exc
            if any(isinstance(q, bool) or not isinstance(q, int)
                   for q in partition_a):
                raise TypeError(
                    "partition_a must contain only integer qubit indices"
                )
            if any(q < 0 or q >= QUBITS for q in partition_a):
                raise ValueError(
                    f"partition_a qubits must be in range 0-{QUBITS - 1}"
                )
            if len(set(partition_a)) != len(partition_a):
                raise ValueError("partition_a must not contain duplicate qubits")
        ...
        # Singular values of M are the nonzero eigenvalues of either M†M
        # or MM†.  Use the smaller side to keep the eigensolver problem small.
        if dim_a < dim_b:
            # Reduced matrix on partition A: G = M M† (dim_a × dim_a).
            G = [[sum(M[i][k] * M[j][k].conjugate() for k in range(dim_b))
                  for j in range(dim_a)] for i in range(dim_a)]
            gram_dim = dim_a
        else:
            # Reduced matrix on complementary partition B: G = M†M.
        ...
        eigenvalues = _hermitian_eigenvalues(G, gram_dim)
    ...
    def fidelity(self, other_state: List[complex]) -> float:
        ...
        other_state = _validate_state_vector(other_state, "other_state")
...
def _hermitian_eigenvalues(A: List[List[complex]], n: int) -> List[complex]:
    """
    Find all eigenvalues of an n×n Hermitian matrix with Jacobi rotations.

    This is a small-matrix, stdlib-only solver for the entropy path. Unlike
    power iteration plus rank-one deflation, Jacobi diagonalization retains
    repeated eigenvalues and does not depend on choosing distinct eigenvectors
    for degenerate eigenspaces. The returned values are the real diagonal of
    the converged Hermitian form; ordering is intentionally unspecified.
    """
    if n <= 0:
        return []
    if n == 1:
        return [complex(A[0][0].real)]

    B = [[complex(A[i][j]) for j in range(n)] for i in range(n)]
    scale = max((abs(B[i][j]) for i in range(n) for j in range(n)), default=0.0)
    tol = max(1e-14, scale * 1e-14)

    # A full sweep visits every off-diagonal pair. The bound is deliberately
    # generous for n <= 8, the supported entropy-matrix size.
    for _ in range(100):
        off_max = max(
            (abs(B[i][j]) for i in range(n) for j in range(i + 1, n)),
            default=0.0,
        )
        if off_max <= tol:
            break
        ...
        for p in range(n - 1):
            for q in range(p + 1, n):
                ...
                # Apply the complex Jacobi similarity rotation to the two
                # affected columns/rows while preserving Hermiticity.
                for k in range(n):
                    if k == p or k == q:
                        continue
                    bkp = c * B[k][p] - s * phase.conjugate() * B[k][q]
                    bkq = s * phase * B[k][p] + c * B[k][q]
                    B[k][p], B[p][k] = bkp, bkp.conjugate()
                    B[k][q], B[q][k] = bkq, bkq.conjugate()

                app = a * c * c - 2.0 * radius * s * c + d * s * s
                aqq = d * c * c + 2.0 * radius * s * c + a * s * s
                B[p][p] = complex(app, 0.0)
                B[q][q] = complex(aqq, 0.0)
                B[p][q] = B[q][p] = 0j

    return [complex(B[i][i].real) for i in range(n)]


def fidelity(state_a: List[complex], state_b: List[complex]) -> float:
    """Fidelity F = |⟨a|b⟩|² between two finite 16-element state vectors."""
    state_a = _validate_state_vector(state_a, "state_a")
    state_b = _validate_state_vector(state_b, "state_b")
    inner = sum(state_a[i].conjugate() * state_b[i] for i in range(STATES))
    return abs(inner) ** 2
...
def chsh_test(shots_per_setting: int = 500) -> str:
    ...
    """
    shots_per_setting = _validate_nonnegative_integer(shots_per_setting, "shots_per_setting", 1)
    settings = [
...
"""),
    ...
    "grover": (
        "Grover search for |0101⟩ (engineering reconstruction, q0 is LSB)",
        """\
INIT
; Engineering reconstruction: one standard Grover iteration on four qubits.
; Target |0101> is index 5 because q0 is the least-significant bit.
; MARK/REFLECT are simulator-only basis operators, not hardware ISA.
; Oracle O = I - 2|5><5|. X1,X3 map target index 5 to index 15.
H 0
H 1
H 2
H 3
...
X 3
; Diffusion D = 2|s><s| - I.
; REFLECT 15 is 2|1111><1111| - I, so H X REFLECT X H = D.
H 0
H 1
H 2
H 3
...
"""),
    ...
    "qft": (
        "Quantum Fourier Transform on 3 qubits (forward, q0 is LSB)",
        """\
INIT
; Engineering reconstruction: forward QFT on q0,q1,q2.
; Convention: F|x> = sum_y exp(+2πi*x*y/8)|y>/sqrt(8), q0 is LSB.
; CP(c,t,theta) = P_c(theta/2) P_t(theta/2) CNOT(c,t)
;                 P_t(-theta/2) CNOT(c,t).
; Prepare q2 first because q0 is the least-significant wire.
H 2
; CP(q1,q2,π/2)
PHASE 1 2
PHASE 2 2
...
CNOT 0 1
H 0
; Reverse-wire swap for the declared integer-index convention.
SWAP 0 2
HALT
"""),
...
MEASURE 0
MEASURE 1
; Classical corrections: X if m1=1, Z if m0=1
IFX 1 2
IFZ 0 2
HALT
"""),
...
═══ QGRIM v2.1 INSTRUCTION SET ════════════════════════════════════════════
 ...
 Software-only extensions (simulator only — not sent to FPGA):
  ...
  NOP IMM=1 IFX       m  q            Simulator-only X(q) if measurement m == 1
  NOP IMM=2 IFZ       m  q            Simulator-only Z(q) if measurement m == 1
  NOP IMM=3 MARK      basis           Simulator-only phase flip of one basis state
  NOP IMM=4 REFLECT   basis           Simulator-only reflection about one basis state
